...
public class NetworkUtil {
     ...
     *
     * @param apiUrl 호출할 OpenAPI URL 주소
     * @param requestHeaders 전송하고 싶은 헤더 주소
     * @return
     */
    public static String get(String apiUrl, @Nullable Map<String, String> requestHeaders) {
        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                return readBody(con.getInputStream()); // 성공 결과값을 문자열로 변환하기
            } else { // 에러 발생
                return readBody(con.getErrorStream()); // 실패 결과값을 문자열로 변환하기
            }
        } catch (IOException e) {
